...
def _risk_level_for_group_id(group_id: Optional[int]) -> str:
    """
    Return a severity label for UI/Discord.
    ...
    if group_id in (30, 485):  # Titans, Supercarriers
        return "critical"
    if group_id in (659, 1972):  # Dreadnoughts, Lancer Dreadnoughts
        return "high"
    if group_id in (547, 1538):  # Carriers, Force Auxiliaries
        return "medium"
    if group_id == 883:  # Capital Industrial Ships (Rorqual)
        return "industrial"
    return "unknown"
...
def _should_alert(alert_level: str) -> bool:
    """
    Whether this entry should produce an alert.

    Industrial is tracked for awareness but does not alert by default.
    """
...
def get_capitals_in_blacklisted_regions(blacklisted_regions):
    ...
    - Risk/alert level + should_alert
    - Region/system + IDs
    - Structure/station name
    - Human breadcrumb "Region → System"

    Keys:
        ownership, character_id, character_name,
        ship_type, ship_type_id, ship_group_id,
        risk, alert_level, should_alert,
        region, region_id, system, system_id,
        structure, location
    """
    ...
    for asset in filtered_assets:
        ...
        # but we follow your existing chain used in this plugin.
        ...
        output.append({
            ...
            # Keep "risk" for backward compatibility, but add explicit keys for clarity.
            "risk": alert_level,
...
def touch_watchlist_last_seen(entries: Sequence[Dict[str, Any]], now=None) -> Tuple[int, int]:
    """
    Ensure watchlist entries exist and update last_seen for any character detected
    with a capital in a blacklisted region.
    ...
    # De-duplicate by CharacterOwnership PK
